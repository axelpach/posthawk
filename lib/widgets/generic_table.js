  ...
  */
    ...
    this.rows.forEach(row => {
      ...
      row.addEventListener('dblclick', (e) => {
        if (e.target.tagName === 'TD') {
          this.onCellDoubleClick(e.target);
        }
      });
    });
  ...
  onCellDoubleClick (cell) {
    ...
    if (text.trim()) {
      ...
      const message = columnHeader ? `${columnHeader} copied to clipboard` : 'Copied to clipboard';
  ...
  getColumnHeader (cell) {
    ...
    if (headerRow && headerRow.children[cellIndex]) {
      return headerRow.children[cellIndex].innerText || headerRow.children[cellIndex].textContent;
    }
    return null;
  }
